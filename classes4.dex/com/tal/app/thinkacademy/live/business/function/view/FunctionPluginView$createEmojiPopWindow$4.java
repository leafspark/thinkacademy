package com.tal.app.thinkacademy.live.business.function.view;

import com.tal.app.thinkacademy.live.core.interfaces.ILiveRoomProvider;
import com.tal.app.thinkacademy.live.core.live.datastorage.DataStorage;
import com.tal.app.thinkacademy.live.core.live.http.response.EmojiDetailEntity;
import java.util.ArrayList;
import kotlin.Metadata;
import kotlin.Unit;
import kotlin.jvm.functions.Function1;
import kotlin.jvm.internal.Lambda;

@Metadata(d1 = {"\u0000\u000e\n\u0000\n\u0002\u0010\u0002\n\u0000\n\u0002\u0010\b\n\u0000\u0010\u0000\u001a\u00020\u00012\u0006\u0010\u0002\u001a\u00020\u0003H\n¢\u0006\u0002\b\u0004"}, d2 = {"<anonymous>", "", "it", "", "invoke"}, k = 3, mv = {1, 6, 0}, xi = 48)
/* compiled from: FunctionPluginView.kt */
final class FunctionPluginView$createEmojiPopWindow$4 extends Lambda implements Function1<Integer, Unit> {
    final /* synthetic */ ArrayList<Integer> $list;
    final /* synthetic */ FunctionPluginView this$0;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    FunctionPluginView$createEmojiPopWindow$4(ArrayList<Integer> arrayList, FunctionPluginView functionPluginView) {
        super(1);
        this.$list = arrayList;
        this.this$0 = functionPluginView;
    }

    public /* bridge */ /* synthetic */ Object invoke(Object obj) {
        invoke(((Number) obj).intValue());
        return Unit.INSTANCE;
    }

    public final void invoke(int i) {
        DataStorage dataStorage;
        if (i == 0) {
            this.$list.clear();
            ILiveRoomProvider liveRoomProvider = this.this$0.getDriver().getLiveRoomProvider();
            EmojiDetailEntity emojiDetailEntity = null;
            if (!(liveRoomProvider == null || (dataStorage = liveRoomProvider.getDataStorage()) == null)) {
                emojiDetailEntity = dataStorage.getEmojiDetailEntity();
            }
            if (emojiDetailEntity != null) {
                emojiDetailEntity.setReportedOverdue(true);
            }
        }
    }
}
