...
public class CancellableContinuationImpl<T> extends DispatchedTask<T> implements CancellableContinuation<T>, CoroutineStackFrame {
    ...
    private final boolean isReusable() {
        return DispatchedTaskKt.isReusableMode(this.resumeMode) && this.delegate.isReusable();
    }
    ...
    private final boolean cancelLater(Throwable th) {
        ...
        return this.delegate.postponeCancellation(th);
    }
    ...
    public final Object getResult() {
        ...
        if (state$kotlinx_coroutines_core instanceof CompletedExceptionally) {
            ...
            if (DebugKt.getRECOVER_STACK_TRACES()) {
                CoroutineStackFrame coroutineStackFrame = (Continuation) this;
                if (coroutineStackFrame instanceof CoroutineStackFrame) {
                    th = StackTraceRecoveryKt.access$recoverFromStackFrame(th, coroutineStackFrame);
                }
            }
            throw th;
        } else if (!DispatchedTaskKt.isCancellableMode(this.resumeMode) || (job = getContext().get(Job.Key)) == null || job.isActive()) {
            return getSuccessfulResult$kotlinx_coroutines_core(state$kotlinx_coroutines_core);
        } else {
            Throwable cancellationException = job.getCancellationException();
            cancelCompletedResult$kotlinx_coroutines_core(state$kotlinx_coroutines_core, cancellationException);
            if (DebugKt.getRECOVER_STACK_TRACES()) {
                CoroutineStackFrame coroutineStackFrame2 = (Continuation) this;
                if (coroutineStackFrame2 instanceof CoroutineStackFrame) {
                    cancellationException = StackTraceRecoveryKt.access$recoverFromStackFrame(cancellationException, coroutineStackFrame2);
                }
            }
    ...
    private final DisposableHandle installParentHandle() {
        ...
        DisposableHandle invokeOnCompletion$default = Job.DefaultImpls.invokeOnCompletion$default(job, true, false, (CompletionHandlerBase) new ChildContinuation(this), 2, (Object) null);
    ...
    private final void releaseClaimedReusableContinuation() {
        ...
        DispatchedContinuation dispatchedContinuation2 = dispatchedContinuation instanceof DispatchedContinuation ? dispatchedContinuation : null;
        if (dispatchedContinuation2 != null && (tryReleaseClaimedContinuation = dispatchedContinuation2.tryReleaseClaimedContinuation(this)) != null) {
            detachChild$kotlinx_coroutines_core();
    ...
    public void resumeUndispatched(CoroutineDispatcher coroutineDispatcher, T t) {
        ...
        DispatchedContinuation dispatchedContinuation2 = dispatchedContinuation instanceof DispatchedContinuation ? dispatchedContinuation : null;
        if (dispatchedContinuation2 != null) {
            coroutineDispatcher2 = dispatchedContinuation2.dispatcher;
        }
    ...
    public Throwable getExceptionalResult$kotlinx_coroutines_core(Object obj) {
        ...
        CoroutineStackFrame coroutineStackFrame = this.delegate;
        return (!DebugKt.getRECOVER_STACK_TRACES() || !(coroutineStackFrame instanceof CoroutineStackFrame)) ? exceptionalResult$kotlinx_coroutines_core : StackTraceRecoveryKt.access$recoverFromStackFrame(exceptionalResult$kotlinx_coroutines_core, coroutineStackFrame);
    }
    ...
    public boolean cancel(Throwable th) {
        ...
        } while (!_state$FU.compareAndSet(this, obj, new CancelledContinuation((Continuation) this, th, z)));
