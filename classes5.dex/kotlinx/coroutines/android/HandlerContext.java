...
public final class HandlerContext extends HandlerDispatcher implements Delay {
    ...
    public void dispatch(CoroutineContext coroutineContext, Runnable runnable) {
        ...
        if (!(!(handler2 instanceof Handler) ? handler2.post(runnable) : AsynchronousInstrumentation.handlerPost(handler2, runnable))) {
            cancelOnRejection(coroutineContext, runnable);
        }
    }

    public DisposableHandle invokeOnTimeout(long j, Runnable runnable, CoroutineContext coroutineContext) {
        if (this.handler.postDelayed(runnable, RangesKt.coerceAtMost(j, DurationKt.MAX_MILLIS))) {
    ...
    public static final void m868invokeOnTimeout$lambda3(HandlerContext handlerContext, Runnable runnable) {
    ...
    public void scheduleResumeAfterDelay(long j, CancellableContinuation<? super Unit> cancellableContinuation) {
        ...
        if (this.handler.postDelayed(handlerContext$scheduleResumeAfterDelay$$inlined$Runnable$1, RangesKt.coerceAtMost(j, DurationKt.MAX_MILLIS))) {
