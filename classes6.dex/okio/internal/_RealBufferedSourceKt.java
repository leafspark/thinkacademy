...
public final class _RealBufferedSourceKt {
    public static final long commonRead(RealBufferedSource realBufferedSource, Buffer buffer, long j) {
        ...
        } else if (realBufferedSource.bufferField.size() == 0 && realBufferedSource.source.read(realBufferedSource.bufferField, (long) Segment.SIZE) == -1) {
            return -1;
        } else {
    ...
    public static final boolean commonExhausted(RealBufferedSource realBufferedSource) {
        ...
        if (!(!realBufferedSource.closed)) {
            throw new IllegalStateException("closed".toString());
        } else if (!realBufferedSource.bufferField.exhausted() || realBufferedSource.source.read(realBufferedSource.bufferField, (long) Segment.SIZE) != -1) {
            return false;
        } else {
            return true;
        }
    ...
    public static final boolean commonRequest(RealBufferedSource realBufferedSource, long j) {
        ...
        if (!(j >= 0)) {
            ...
            while (realBufferedSource.bufferField.size() < j) {
                if (realBufferedSource.source.read(realBufferedSource.bufferField, (long) Segment.SIZE) == -1) {
                    return false;
                }
            }
            return true;
    ...
    public static final int commonSelect(RealBufferedSource realBufferedSource, Options options) {
        ...
        if (!realBufferedSource.closed) {
            ...
            } while (realBufferedSource.source.read(realBufferedSource.bufferField, (long) Segment.SIZE) != -1);
            return -1;
        }
        throw new IllegalStateException("closed".toString());
    }
    ...
    public static final int commonRead(RealBufferedSource realBufferedSource, byte[] bArr, int i, int i2) {
        ...
        if (realBufferedSource.bufferField.size() == 0 && realBufferedSource.source.read(realBufferedSource.bufferField, (long) Segment.SIZE) == -1) {
            return -1;
        }
            ...
            if (z) {
                while (j < j2) {
                    ...
                    if (indexOf == -1) {
                        ...
                        if (size >= j2 || realBufferedSource.source.read(realBufferedSource.bufferField, (long) Segment.SIZE) == -1) {
                            break;
                        }
                        j = Math.max(j, size);
                    } else {
