...
public final class SelectBuilderImpl<R> extends LockFreeLinkedListHead implements SelectBuilder<R>, SelectInstance<R>, Continuation<R>, CoroutineStackFrame {
    ...
    private final void initCancellability() {
        ...
        if (job != null) {
            DisposableHandle invokeOnCompletion$default = Job.DefaultImpls.invokeOnCompletion$default(job, true, false, (CompletionHandlerBase) new SelectOnCancelling(), 2, (Object) null);
    ...
    public final void handleBuilderException(Throwable th) {
        if (trySelect()) {
            ...
            ((Continuation) this).resumeWith(Result.constructor-impl(ResultKt.createFailure(th)));
                ...
                if (obj2 == SelectKt.UNDECIDED) {
                    ...
                    if (Result.isFailure-impl(obj)) {
                        ...
                        if (DebugKt.getRECOVER_STACK_TRACES() && (coroutineStackFrame instanceof CoroutineStackFrame)) {
                            r4 = StackTraceRecoveryKt.recoverFromStackFrame(r4, coroutineStackFrame);
                        }
                        coroutineStackFrame.resumeWith(Result.constructor-impl(ResultKt.createFailure(r4)));
                        return;
                    }
    ...
    public void resumeSelectWithException(Throwable th) {
        if (!DebugKt.getASSERTIONS_ENABLED() || isSelected()) {
            while (true) {
                ...
                if (obj == SelectKt.UNDECIDED) {
                    CoroutineStackFrame coroutineStackFrame = this.uCont;
                    if (_result$FU.compareAndSet(this, SelectKt.UNDECIDED, new CompletedExceptionally((!DebugKt.getRECOVER_STACK_TRACES() || !(coroutineStackFrame instanceof CoroutineStackFrame)) ? th : StackTraceRecoveryKt.recoverFromStackFrame(th, coroutineStackFrame), false, 2, (DefaultConstructorMarker) null))) {
                        return;
                    }
                } else if (obj != IntrinsicsKt.getCOROUTINE_SUSPENDED()) {
                    throw new IllegalStateException("Already resumed");
                    ...
                    Result.Companion companion = Result.Companion;
                    intercepted.resumeWith(Result.constructor-impl(ResultKt.createFailure(th)));
                    return;
                }
            }
        } else {
