...
public class IOUtils {
    ...
    public static long skip(InputStream inputStream, long j) throws IOException {
        if (j >= 0) {
            ...
            long j2 = j;
            while (j2 > 0) {
                long read = (long) inputStream.read(SKIP_BYTE_BUFFER, 0, (int) Math.min(j2, IjkMediaMeta.AV_CH_TOP_CENTER));
                if (read < 0) {
                    break;
                }
                j2 -= read;
    ...
    public static long skip(ReadableByteChannel readableByteChannel, long j) throws IOException {
        if (j >= 0) {
            ByteBuffer allocate = ByteBuffer.allocate((int) Math.min(j, IjkMediaMeta.AV_CH_TOP_CENTER));
            long j2 = j;
            while (j2 > 0) {
                allocate.position(0);
                allocate.limit((int) Math.min(j2, IjkMediaMeta.AV_CH_TOP_CENTER));
    ...
    public static long skip(Reader reader, long j) throws IOException {
        if (j >= 0) {
            ...
            long j2 = j;
            while (j2 > 0) {
                long read = (long) reader.read(SKIP_CHAR_BUFFER, 0, (int) Math.min(j2, IjkMediaMeta.AV_CH_TOP_CENTER));
                if (read < 0) {
                    break;
                }
                j2 -= read;
