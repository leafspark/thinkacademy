...
public class DualECSP800DRBG implements SP80090DRBG {
    ...
    static {
        ...
        dualECPointsArr[1] = new DualECPoints(BERTags.PRIVATE, fp2.createPoint(bigInteger5, bigInteger6), fp2.createPoint(bigInteger7, bigInteger8), 1);
    ...
    public DualECSP800DRBG(DualECPoints[] dualECPointsArr, Digest digest, int i, EntropySource entropySource, byte[] bArr, byte[] bArr2) {
        ...
        if (Utils.isTooLarge(bArr, 512)) {
            ...
            throw new IllegalArgumentException("EntropySource must provide between " + i + " and " + _BufferKt.SEGMENTING_THRESHOLD + " bits");
        } else {
    ...
    public int generate(byte[] bArr, byte[] bArr2, boolean z) {
        ...
        } else if (this._reseedCounter + ((long) length2) > 2147483648L) {
            return -1;
        } else {
            if (z) {
                reseed(bArr2);
