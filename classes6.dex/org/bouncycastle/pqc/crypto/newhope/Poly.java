...
class Poly {
    ...
    static void fromBytes(short[] sArr, byte[] bArr) {
        for (int i = 0; i < 256; i++) {
            ...
            int i3 = i * 4;
            sArr[i3 + 0] = (short) ((bArr[i2 + 0] & 255) | ((b & Utf8.REPLACEMENT_BYTE) << 8));
    ...
    static void getNoise(short[] sArr, byte[] bArr, byte b) {
        ...
        bArr2[0] = b;
        byte[] bArr3 = new byte[_BufferKt.SEGMENTING_THRESHOLD];
        ChaCha20.process(bArr, bArr2, bArr3, 0, _BufferKt.SEGMENTING_THRESHOLD);
        for (int i = 0; i < 1024; i++) {
