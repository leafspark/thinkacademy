...
public class PerformanceTracker {
    ...
    public void logRenderTimes() {
        if (this.enabled) {
            ...
            for (int i = 0; i < sortedRenderTimes.size(); i++) {
                ...
                Log.d(L.TAG, String.format("\t\t%30s:%.2f", new Object[]{pair.first, pair.second}));
            }
        }
    }
    ...
    public List<Pair<String, Float>> getSortedRenderTimes() {
        ...
        for (Map.Entry next : this.layerRenderTimes.entrySet()) {
            arrayList.add(new Pair((String) next.getKey(), Float.valueOf(((MeanCalculator) next.getValue()).getMean())));
        }
